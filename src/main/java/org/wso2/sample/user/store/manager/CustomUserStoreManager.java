        ...
        try {
            ...
            if (rs.next()) {
                ...
                boolean requireChange = rs.getBoolean(5);
                   ...
                   java.sql.Date date = (java.sql.Date) gc.getTime();
                   Long timeStamp = null;
                   if (!(requireChange && changedTime.before(date))) {
                        ...
                        try {
                            ...
                            String claimTime = this.getClaimAtrribute(LAST_PASSWORD_RESET_CLAIM,"lastPasswordResetTimestamp","http://wso2.org/claims/");
                            timeStamp = Long.parseLong(claimTime);  
                            if(timeStamp != null)
                            {
                                Timestamp time = new Timestamp(timeStamp);
                                java.sql.Date passwordResetDate = new java.sql.Date(time.getTime());
                                long duration = date.getTime() - passwordResetDate.getTime();
                                if(TimeUnit.DAYS.convert(duration, TimeUnit.DAYS) > 65)
                                {
                                    log.error("Password is expired please reset the password");
                                    throw new UserStoreException("Password is expired please reset the password");
                                }
                                else{
                                    isAuthed = (password.trim().equals(storedPassword));
                                }
                            }
                            else{
                                isAuthed = (password.trim().equals(storedPassword));
                            ...
                            log.error("User Exception Occur "+ex.getLocalizedMessage());
                        }
